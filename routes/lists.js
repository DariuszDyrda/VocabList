...
router.get('/index/:id/pdfexport', (req, res) => {
  List.findById(req.params.id).populate('words').exec(async function (err, list) {
    if (err) {
      console.log(err);
    } else {
      await ejs.renderFile('views/lists/pdfexportTemplate.ejs', {
        list: list,
        user: req.user
        ...
        if (err) {
          ...
          await pdf.create(str, options).toStream(function (err, stream) {
            if (err) {
              console.log(err);
              res.redirect('/index/'+req.params.id);
            } else {
